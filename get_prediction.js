...
const mainUrl = 'https://www.playsport.cc/predictgame.php?action=scale&'
...
const sportKind = allianceidMap[argvs[0]]
const date = argvs[1]

function getInfo () {
  const url = `${mainUrl}allianceid=${sportKind}&gametime=${date}&sid=${sidArray[0]}`
...
function processTop100Url(matches) {
  const url2 = `${mainUrl}allianceid=${sportKind}&gametime=${date}&sid=${sidArray[1]}`
...
function show (matches) {
  for (let i in (matches)) {
    ...
    console.log(
      matches[i].away_win,
      matches[i].home_win,
      matches[i].handicap_weak,
      matches[i].handicap_strong,
      matches[i].total_big,
      matches[i].total_small
    )
  }
}
