...
async function getPrediction (sport) {
  ...
  const top100Info = await getTop100Info()
  const matches = await processTeams(info)
  const finalResult = await processTop100Teams(top100Info, matches)

  return finalResult

  // 1. 取得賽事內容
  async function getInfo () {
    const url = `${PLAY_SPORT_URL}allianceid=${sportKind}&gametime=${date}&sid=${SID_MAP[0]}`
  ...
  // 2. 取得前100高手預測資訊
  async function getTop100Info () {
    const url2 = `${PLAY_SPORT_URL}allianceid=${sportKind}&gametime=${date}&sid=${SID_MAP[1]}`

    const body = await rp({url: url2})
  ...
  // 3. 取得一般預測人數
  async function processTeams (body) {
    let allMatch = []
    let matchId = 0
    let match = ''
    let awayWin = 0
    ...
    for (let i in body) {
      let teamName = body[i].split('</a')[0].split('>')[1].trim()

      if (!g1TeamList.includes(teamName)) {
        g1TeamList = g1TeamList + teamName
      } else {
        teamName = teamName + '(G2)'
      }
  ...
  // 4. 取得前100高手預測人數
  async function processTop100Teams (body, matches) {
    let awayWin = 0
    let homeWin = 0
    let handicapWeak = 0
    let handicapStrong = 0
