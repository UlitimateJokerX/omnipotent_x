 ...
 * @param {Boolean} checkLength 是否為查詢資料筆數
 *
 * @return {Array}
 */
async function getData (table, criteria, fuzzy = false, checkLength = false) {
 ...
 *
 * @param {String} table         資料表名稱
 * @param {Object} primaryColumn 主鍵欄位與數值 ex: {id: 1}
 * @param {Object} newData       欲更新的欄位與數值 ex: {message: 'abc'}
 *
 * @return {Array}
 */
async function updateData (table, primaryColumn, newData) {
  const doc = await loadDoc()
  const sheet = doc.sheetsByTitle[table]
  let rows = await sheet.getRows()

  const rowData = rows.filter(row => {
    if (row[Object.keys(primaryColumn)[0]] == primaryColumn[Object.keys(primaryColumn)[0]]) {
      return row
    }
  })

  rowData[0]['deleted'] = 1
  await rowData[0].save()
}
